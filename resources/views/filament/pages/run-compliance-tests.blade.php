<x-filament-panels::page>
    <div x-data="complianceTestRunner()" x-init="init()" class="space-y-6">
        {{-- Controls --}}
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-4">
                <x-filament::button
                    ...
                    color="primary"
                >
                    <span x-show="!running">Start Compliance Tests</span>
                ...
                <a href="{{ \App\Filament\Resources\ComplianceReportResource::getUrl() }}">
                    <x-filament::button
                        color="gray"
        ...
        <div x-show="running && currentTest" x-cloak
            ...
            <div>
                <div class="font-medium text-primary-700 dark:text-primary-300" x-text="'Testing: ' + currentTest"></div>
                ...
                <table class="w-full text-sm">
                    <thead class="bg-gray-50 dark:bg-gray-700/50">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left font-medium text-gray-500 dark:text-gray-400">Section</th>
                            <th class="px-4 py-3 text-left font-medium text-gray-500 dark:text-gray-400">Test File</th>
                            ...
                            <th class="px-4 py-3 text-left font-medium text-gray-500 dark:text-gray-400">Compliance</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                        <template x-for="(r, idx) in results" :key="idx">
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors"
                                ...
                                <td class="px-4 py-2.5">
                                    <span class="inline-flex px-2 py-0.5 rounded-md text-xs font-medium bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-300"
                                          x-text="r.test_category"></span>
                                </td>
                                ...
                                <td class="px-4 py-2.5">
                                    <span x-show="r.compliance_status"
                                          class="inline-flex px-2 py-0.5 rounded-md text-xs font-medium"
                                          x-bind:class="{
                                              'bg-success-100 text-success-700 dark:bg-success-900/30 dark:text-success-400': r.compliance_status === 'Compliant',
                                              'bg-danger-100 text-danger-700 dark:bg-danger-900/30 dark:text-danger-400': r.compliance_status === 'Non-Compliant'
                                          }"
                                          x-text="r.compliance_status">
                                    </span>
                                    <span x-show="r.error && !r.compliance_status"
                                          class="text-xs text-danger-500 truncate max-w-[200px] inline-block"
                                          x-text="r.error"
                                          x-bind:title="r.error">
                                    </span>
                                </td>
                            </tr>
                        </template>
        ...
        <div x-show="!running && results.length === 0 && !loading" class="text-center py-12">
            ...
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                Click "Start Compliance Tests" to run the Isartor PDF/A-1b test suite.
                <br>Each test will be processed individually and results shown in real-time.
            </p>
        </div>
            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    <div class="font-semibold"
                         ...
                         x-text="'Test Run Complete — ' + passedCount + ' passed, ' + failedCount + ' failed, ' + errorCount + ' errors out of ' + completedCount + ' tests'">
                    </div>
                    <div class="text-sm mt-1"
                         x-bind:class="failedCount === 0 && errorCount === 0 ? 'text-success-600 dark:text-success-400' : 'text-danger-600 dark:text-danger-400'">
                        Results have been saved. <a :href="'{{ \App\Filament\Resources\ComplianceReportResource::getUrl() }}'" class="underline">View in Compliance Reports</a>
                    </div>
                </div>
            </div>
        </div>
            ...
            return {
                ...
                async startTests() {
                    ...
                    try {
                        ...
                        const listResponse = await fetch('/compliance/test-files');
                        ...
                        for (let i = 0; i < this.files.length; i++) {
                            ...
                            try {
                                const response = await fetch('/compliance/run-single-test', {
                                    method: 'POST',
                                    headers: {
