#!/usr/bin/env python3
"""
new_annotation_writer.py

New annotation writer — based on apply_annotations_direct_new.py.
Intended as the development branch for annotation rendering improvements.
Use compare_first_annotation.py to verify it produces the same output as
the reference writer before making any changes.
"""
...
def resolve_annotation_font_weight(ann: Dict[str, Any]) -> str:
    ...
    # Detect bold/demi-bold weight from the font source name when fontWeight is
    # stored as a normal weight (e.g. ITCFranklinGothicStd-Demi has weight=400
    # in the DB but is visually a semi-bold font).  Check for well-known bold
    # weight suffixes in the font family or source name fields.
    for _fw_key in ("fontSourceName", "fontFamily"):
        _fw_raw = str(ann.get(_fw_key) or "")
        if not _fw_raw:
            continue
        # Strip any embedded-font ABCDEF+ subset prefix.
        _fw_name = _fw_raw.split("+", 1)[-1] if "+" in _fw_raw else _fw_raw
        # Normalize: remove separators, lower-case.
        _fw_lower = _fw_name.replace("-", "").replace("_", " ").lower()
        if any(
            indicator in _fw_lower
            for indicator in ("demi", "demibold", "semibold")
        ):
            return "700"
...
def draw_text_using_exact_source_lines(
    ...
    for line_entry in lines:
        ...
        # If the substitute font renders wider than the source line rect, scale the
        # font size down proportionally so text never overflows past the page edge.
        _line_avail_w = line_rect.x1 - draw_x
        if _line_avail_w > 1.0:
            _measured_w = line_font.text_length(line_text, fontsize=line_font_size)
            if _measured_w > _line_avail_w:
                _scale = _line_avail_w / _measured_w
                line_font_size *= _scale
                text_width = line_font.text_length(line_text, fontsize=line_font_size)
                if line_entry.get("baseline_y") is None:
                    baseline_y = line_rect.y0 + (line_font_ascender * line_font_size)
...
def draw_text_using_exact_source_spans(
    ...
    for line_entry in lines:
        ...
        for span_entry in line_entry.get("spans") or []:
            ...
            # Fix #2: Fall back to DejaVu Sans for glyphs missing from the
            # substitute font.  Pi/symbol fonts (e.g. UniversalStd-NewswithCommPi)
            # contain characters like U+25B6 (▶) that common Latin fonts lack.
            # DejaVu Sans covers a broad Unicode range and is available system-wide.
            _DEJAVU_PATH = "/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf"
            if os.path.exists(_DEJAVU_PATH) and any(
                span_font.has_glyph(ord(c)) == 0
                for c in span_text
                if ord(c) > 127
            ):
                span_font = fitz.Font(fontfile=_DEJAVU_PATH)
                span_fontname = "DejaVuSans"
                page.insert_font(fontname=span_fontname, fontfile=_DEJAVU_PATH)
                span_fontfile = _DEJAVU_PATH
            ...
            if _prev_span_rect is not None and _prev_span_text is not None:
                ...
                if _adj_gap < 0.5:
                    ...
                    # Only apply when the previous span is a short alphanumeric label
                    # (field number like "1", "6a", etc.) so we don't accidentally
                    # shift text that follows a URL, a sentence fragment, or another
                    # long span where the excess comes from font-metric differences
                    # rather than stripped leading whitespace.
            ...
            # If the substitute font renders wider than the source span rect, scale the
            # font size down proportionally so text never overflows past the page edge.
            ...
            if _span_avail_w > 1.0:
                ...
                if _measured_w > _span_avail_w:
                    ...
                    # that trailing whitespace stripped from the span text is not
                    # "consumed" by the scaled glyph run.  PDF text extractors often
                    # strip trailing spaces from a span's text while keeping the span
                    # bbox wide enough to include that stripped space's advance width.
                    # Scaling to span_avail_w exactly pushes the last visible glyph
                    # flush to x1, erasing the visual gap that should separate it from
                    # the next adjacent span (e.g. "…beneficial owner." → "I agree").
                    # Reserving space_w leaves the stripped space room to breathe.
                    _space_w_reserve = span_font.text_length(" ", fontsize=span_font_size)
                    _scale_target = max(
                        _span_avail_w - _space_w_reserve,
                        _span_avail_w * 0.95,
                    )
                    _scale = _scale_target / _measured_w
                    span_font_size *= _scale
                    # Re-compute baseline_y if it was derived from font metrics rather
                    # than stored origin coordinates.
                    if span_entry.get("baseline_y") is None:
                        baseline_y = span_rect.y0 + (float(getattr(span_font, "ascender", 0.8) or 0.8) * span_font_size)
                else:
                    # Fix #3: Word-spacing justification when the substitute font
                    # renders narrower than the source bbox.  Arimo is typically a
                    # few points narrower than HelveticaNeueLTStd for the same text.
                    # Distributing the deficit as extra inter-word spacing preserves
                    # the horizontal position of the trailing glyph and matches the
                    # spacing visible in the original PDF.  Only apply when the extra
                    # per-space is a small nudge (< 3 pt) to avoid over-stretching
                    # single-space spans where the original font is just wider overall.
                    #
                    # Fix #3b: Reserve one space-character width at the right edge of
                    # the fill target.  PDF text extractors strip trailing whitespace
                    # from span text while keeping the span bbox wide enough to
                    # include the stripped space glyph's advance width.  If we fill
                    # all the way to span_rect.x1, the last drawn character occupies
                    # ground that belongs to the stripped trailing space, removing the
                    # visual gap between adjacent spans (e.g. "...beneficial owner."
                    # and "I agree...").  Targeting span_rect.x1 minus one space
                    # width restores that gap.
                    _space_count = span_text.count(" ")
                    if _space_count > 0 and _measured_w < _span_avail_w - 2.0:
                        _space_w_estimate = span_font.text_length(" ", fontsize=span_font_size)
                        _extra_per_space = max(
                            0.0,
                            (_span_avail_w - _measured_w - _space_w_estimate) / _space_count,
                        )
                        if 0.0 < _extra_per_space < 3.0:
                            _use_word_justify = True
                            _word_extra_spacing = _extra_per_space

            # If the source span carries its own rotation (e.g. vertical text extracted
            # from a rotated PDF glyph stream), build a per-span morph using the span
            # origin as the pivot.
            span_rotation_raw = float(span_entry.get("span_rotation") or 0.0)
            if abs(span_rotation_raw) > 1e-6:
                span_morph_angle = normalize_rotation_degrees(span_rotation_raw)
                effective_morph = build_rotation_morph(
                    span_morph_angle, fitz.Point(draw_x, baseline_y)
                )
            else:
                effective_morph = morph
...
def _resolve_promoted_source_span_rects(ann: Dict[str, Any]) -> list[fitz.Rect]:
    source_spans = ann.get("sourceSpans")
    ...
    for span in source_spans:
        ...
        if rect.is_empty:
            continue
        span_rects.append(rect)
    return span_rects


def _promoted_source_prefers_span_replay(ann: Dict[str, Any]) -> bool:
    source_spans = ann.get("sourceSpans")
    if not isinstance(source_spans, list) or len(source_spans) < 2:
        return False

    reversed_or_rotated_count = 0
    valid_span_count = 0
    for span in source_spans:
        if not isinstance(span, dict):
            continue
        valid_span_count += 1
        prefers_span_replay = False
        try:
            if abs(float(span.get("rotation") or 0.0)) > 1e-6:
                prefers_span_replay = True
        except Exception:
            pass
        direction = span.get("direction")
        if isinstance(direction, (list, tuple)) and len(direction) >= 1:
            try:
                if float(direction[0]) < 0.0:
                    prefers_span_replay = True
            except Exception:
                pass
        if prefers_span_replay:
            reversed_or_rotated_count += 1

    if valid_span_count > 0 and reversed_or_rotated_count == valid_span_count:
        return True

    clip_rect = _resolve_promoted_source_clip_rect(ann)
    if clip_rect is not None and not clip_rect.is_empty and clip_rect.height <= 10.0 and reversed_or_rotated_count > 0:
        return True
...
def _rects_nearly_equal(rect_a: fitz.Rect, rect_b: fitz.Rect, tolerance: float = 0.25) -> bool:
    if rect_a.is_empty or rect_b.is_empty:
        return False
    return (
        abs(float(rect_a.x0) - float(rect_b.x0)) <= tolerance
        and abs(float(rect_a.y0) - float(rect_b.y0)) <= tolerance
        and abs(float(rect_a.x1) - float(rect_b.x1)) <= tolerance
        and abs(float(rect_a.y1) - float(rect_b.y1)) <= tolerance
    )
...
def should_replay_promoted_source_block(ann: Dict[str, Any], rect: Optional[fitz.Rect], rotation: float) -> bool:
    ...
    if _is_geometrically_safe_show_pdf_page_candidate(ann, source_clip_rect, rect):
        return True
    if _rects_nearly_equal(source_clip_rect, rect):
        return True
...
def replay_promoted_source_block(page: fitz.Page, ann: Dict[str, Any], rect: fitz.Rect) -> bool:
    ...
    span_rects = _resolve_promoted_source_span_rects(ann)
    replay_pad = max(0.0, float(PROMOTED_SOURCE_REPLAY_PAD))
    source_clip_rect = fitz.Rect(clip_rect)
    dest_rect = fitz.Rect(rect)
    rects_aligned = _rects_nearly_equal(source_clip_rect, dest_rect)

    # Single-line promoted blocks that mix multiple spans or symbol fonts can
    # accumulate thin replay seams when the entire block is imported as one clip.
    # Replaying each source span independently preserves the original glyph edges
    # more faithfully while still mapping into the saved destination geometry.
    if len(line_rects) <= 1 and len(span_rects) >= 2 and _promoted_source_prefers_span_replay(ann):
        rendered_any_span = False
        try:
            for span_rect in span_rects:
                mapped_rect = _map_source_rect_to_dest_rect(span_rect, source_clip_rect, dest_rect)
                if mapped_rect is None or mapped_rect.is_empty:
                    continue
                ...
                rendered_any_span = True
            if rendered_any_span:
                return True
        except Exception:
            pass

    if len(line_rects) >= 2 and not rects_aligned:
