...
class ApiIntegrations extends Page
{
    ...
    public string $wordConversionProvider = DocumentConversionSetting::PROVIDER_LOCAL;

    public string $excelConversionProvider = DocumentConversionSetting::PROVIDER_LOCAL;
    ...
    public function mount(): void
    {
        ...
        $this->wordConversionProvider = $settings->word_provider;
        $this->excelConversionProvider = $settings->excel_provider;
        $this->fallbackToLocal = $settings->fallback_to_local;
        $this->refresh();
    }
    ...
    public function saveDocumentConversionSettings(): void
    {
        $validated = $this->validate([
            'wordConversionProvider' => ['required', 'in:local,adobe'],
            'excelConversionProvider' => ['required', 'in:local,adobe'],
            'fallbackToLocal' => ['boolean'],
        ]);

        DocumentConversionSetting::current()->update([
            'word_provider' => $validated['wordConversionProvider'],
            'excel_provider' => $validated['excelConversionProvider'],
            'fallback_to_local' => $validated['fallbackToLocal'],
        ]);

        $this->integrations['adobe'] = $this->fetchAdobeStatus();

        Notification::make()
            ->title('Document conversion providers saved')
            ->success()
            ->send();
    }
    ...
    private function fetchAdobeStatus(): array
    {
        $service = app(AdobePdfServices::class);
        $extra = [
            'Region' => strtoupper((string) config('services.adobe_pdf_services.region', 'US')),
            'Word Provider' => ucfirst($this->wordConversionProvider),
            'Excel Provider' => ucfirst($this->excelConversionProvider),
            'Customer Charge' => sprintf(
                '$%.2f / %d pages',
                (float) config('document-conversion.price_usd_per_transaction', 0.10),
                (int) config('document-conversion.pages_per_transaction', 50),
            ),
        ];

        if (! $service->configured()) {
            return array_merge($this->noKey(), ['extra' => $extra]);
        }
        ...
        try {
            ...
            return [
                ...
                'balance' => null,
                'label' => 'Connected',
                'note' => 'Credentials accepted. Export entitlement is checked when a conversion runs.',
                'extra' => $extra,
            ];
        } catch (\Throwable $e) {
            return [
