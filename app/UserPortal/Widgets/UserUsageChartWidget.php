...
class UserUsageChartWidget extends ChartWidget
{
    ...
    protected function getData(): array
    {
        ...
        [$startDate, $groupFormat, $labelFormat, $interval] = match ($this->filter) {
        ...
        $services = $baseQuery->clone()->distinct()->pluck('service')->toArray();
        ...
        $end = now();
        while ($cursor <= $end) {
            $key = $cursor->format(match ($interval) {
                'hour' => 'Y-m-d H:00',
                'day' => 'Y-m-d',
                'month' => 'Y-m',
            });
            $allLabels[$key] = $cursor->format($labelFormat);
